...
stmt_execute_metadata_t * MySQL_Protocol::get_binds_from_pkt(void *ptr, unsigned int size, MySQL_STMT_Global_info *stmt_info, stmt_execute_metadata_t **stmt_meta, StmtLongDataHandler *sldh) {
	...
	if (num_params) {
		...
		for (i=0;i<num_params;i++) {
			unsigned long *_l = 0;
			my_bool * _is_null;
			void *_data = sldh->get(ret->stmt_id, i, &_l, &_is_null);
			if (_data) {
